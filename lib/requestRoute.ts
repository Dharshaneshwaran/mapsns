...
export async function requestRoute(start: Coordinate, end: Coordinate, mode: TravelMode, signal: AbortSignal, heading: number | null = null): Promise<Pick<WalkingRoute, "points" | "distanceMeters" | "durationSeconds">> {
  ...
  const wait = Math.max(0, nextRequest - Date.now()); nextRequest = Date.now() + wait + 1100;
  ...
  const response = await fetch(`${root}/${start.lng},${start.lat};${end.lng},${end.lat}?overview=full&geometries=geojson${heading !== null ? `&bearings=${Math.round((heading + 360) % 360)},90;` : ""}`, { signal });
