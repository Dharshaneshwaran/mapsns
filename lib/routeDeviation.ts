...
export function routeDeviation(position: Point, points: Point[], heading: number | null) {
  ...
  let distance = Infinity, direction = 0;
  for (let i = 1; i < points.length; i++) {
    ...
    if (next < distance) { distance = next; direction = (Math.atan2(dx, dy) * 180 / Math.PI + 360) % 360; }
  }
  return { distance, wrongWay: heading !== null && Number.isFinite(heading) && Math.abs(((heading - direction + 540) % 360) - 180) > 110 };
}
