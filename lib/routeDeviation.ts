...
export function routeDeviation(position: Point, points: Point[], heading: number | null) {
  ...
  return { distance, remainingFraction: total > 0 ? Math.max(0, 1 - progress / total) : 0, wrongWay: heading !== null && Number.isFinite(heading) && Math.abs(((heading - direction + 540) % 360) - 180) > 110 };
}
