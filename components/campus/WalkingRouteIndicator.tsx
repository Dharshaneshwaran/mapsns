"use client";

import { WalkingRoute } from "@/types/campus";

type Props = {
  route: WalkingRoute | null;
  isWalking: boolean;
};

export default function WalkingRouteIndicator({ route, isWalking }: Props) {
  if (!route || isWalking) return null;

  return (
    <div className="absolute top-20 sm:top-24 left-1/2 -translate-x-1/2 z-30 safe-top">
      <div className="bg-white rounded-2xl shadow-lg px-4 py-2.5 flex items-center gap-3">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-zinc-400" />
          <span className="text-xs font-semibold text-zinc-700">Route ready</span>
        </div>
        <div className="w-px h-4 bg-zinc-200" />
        <span className="text-xs text-zinc-500 max-w-48 truncate">{route.name}</span>
        {route.isPrototype && (
          <>
            <div className="w-px h-4 bg-zinc-200" />
            <span className="text-[10px] text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">
              Demo
            </span>
          </>
        )}
      </div>
    </div>
  );
}
