...
import { ArrowUpRight, Bookmark, Building2, MapPin, Navigation, Settings, ArrowLeft, Grid3x3, ChevronRight } from "lucide-react";
        ...
        {morePlaces.map((item, index) => {
          ...
          return <button key={`${item.title}-${index}`} className="campus-place-tile event-tile" onClick={() => matchedPlace && onSelect(matchedPlace)} disabled={!matchedPlace}>
            <span className="campus-place-art">
              {item.icon ? <Image src={item.icon} alt="" width={160} height={100} unoptimized className="object-cover rounded-xl" /> : <Building2 size={48} strokeWidth={1.2} />}
