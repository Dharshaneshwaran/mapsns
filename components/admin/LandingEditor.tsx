...
export default function LandingEditor() {
  ...
  return <section className="mb-7 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-semibold">Landing sidebar</h2><p className="mt-1 text-sm text-zinc-500">Edit the introduction, featured places and advertisement shown on desktop and mobile.</p>
    ...
    </div><div className="space-y-3"><label className="flex gap-2 text-sm font-semibold"><input type="checkbox" checked={draft.ad.enabled} onChange={(event) => { setDraft({ ...draft, ad: { ...draft.ad, enabled: event.target.checked } }); setMessage(""); }} />Show landing advertisement</label>
      {([['eyebrow', 'Ad label', 50], ['title', 'Ad title', 100], ['description', 'Ad description', 280], ['imageUrl', 'Image path or URL', 500], ['linkUrl', 'Destination URL', 500], ['buttonLabel', 'Button text', 40]] as const).map(([key, label, max]) => <label key={key} className="block text-sm font-medium">{label}<input maxLength={max} value={draft.ad[key]} onChange={(event) => { setDraft({ ...draft, ad: { ...draft.ad, [key]: event.target.value } }); setMessage(""); }} className="mt-1 block w-full rounded-lg border border-zinc-300 p-2 font-normal" /></label>)}
