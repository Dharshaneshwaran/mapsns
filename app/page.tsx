...
function CampusMapApp({ initialProfile }: { initialProfile: UserProfile }) {
  ...
  useEffect(() => {
    if (!rerouteMessage || rerouteMessage.startsWith("Updating")) return;
    const hold = rerouteMessage.startsWith("Route updated") ? 3000 : 8000;
    const timer = setTimeout(() => setRerouteMessage(""), hold);
